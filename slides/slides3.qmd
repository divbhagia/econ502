...
$$C(r, w, Q) = wL^* + rK^* = w \cdot \frac{Q}{\sqrt{w/r}} + r \cdot Q \sqrt{w/r} = 2Q\sqrt{rw}$$
...
colors = ['#a23b72', '#2e86ab', '#f18f01', '#c73e1d']
...
ax.plot(Q, smc, linewidth=2.5, color=colors[0], label='$SMC$', linestyle='--')
ax.plot(Q, savc, linewidth=2.5, color=colors[1], label='$SAVC$')
ax.plot(Q, sac, linewidth=2.5, color=colors[2], linestyle='--', label='$SAC$')

# Min SAVC (shutdown price)
q_shut = 5  # min of SAVC at Q = -(-1)/(2*0.1) = 5
p_shut = 0.1*25 - 5 + 5
ax.axhline(y=p_shut, color=colors[3], linewidth=1, linestyle=':', alpha=1)
...
mask = smc >= savc
ax.plot(Q[mask], smc[mask], linewidth=4, color=colors[0], alpha=1.0, label='Supply curve')
ax.plot([0, 0], [0, p_shut-0.1], linewidth=8, color=colors[0], zorder=10)
