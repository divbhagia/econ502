...
ax.plot(x_orig, y_orig, 'gray', linewidth=2.5, linestyle='--', label=f'Original Budget: {px}x + {py}y = {I}', alpha=0.7)

# Cash transfer budget
x_cash = np.linspace(0, (I+transfer)/px, 100)
y_cash = (I + transfer - px*x_cash) / py
ax.plot(x_cash, y_cash, 'black', linewidth=3, label=f'Cash Transfer (${transfer})')
...
ax.plot(x_inkind, y_inkind, 'red', linewidth=10, label=f'In-Kind Transfer (${transfer} vouchers for X)', alpha=0.35)
...
ax.text(x_min-0.3, 12, f'Must buy\n≥{x_min:.0f} units', fontsize=10, color='red', ha='right', fontweight='bold')
