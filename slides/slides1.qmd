...
# Create plot
fig, ax = plt.subplots(figsize=(5.75, 3.9))
...
Along an indifference curve, utility is constant: $U(x,y) = \bar{U}$. Taking the total differential:
...
# Create figure
fig, ax = plt.subplots(figsize=(8, 5.85))
...
ax.set_xlabel('Chase Coffee', fontsize=16)
ax.set_ylabel('Starbucks Coffee', fontsize=16)
ax.set_xlim(0, 15)
ax.set_ylim(0, 32)
ax.grid(True, alpha=0.3)
...
$$MU_x = \frac{\partial U}{\partial x} = \alpha x^{\alpha-1}y^{\beta}, \quad \quad MU_y = \frac{\partial U}{\partial y} = \beta x^{\alpha}y^{\beta-1}$$
...
ax.plot(x, y, '-', linewidth=2, label=f'Budget Line: {px}x + {py}y = {I}', color='black')
...
Denote $\lambda$ as the **Lagrange multiplier** and setup the **Lagrangian**:
$$L(x,y,\lambda) = U(x,y) + \lambda(I - p_x x - p_y y)$$

**First-order conditions** (FOCs):

1. $\frac{\partial L}{\partial x} = \frac{\partial U}{\partial x} - \lambda p_x = 0$
2. $\frac{\partial L}{\partial y} = \frac{\partial U}{\partial y} - \lambda p_y = 0$
3. $\frac{\partial L}{\partial \lambda} = I - p_x x - p_y y = 0$
...
This is exactly the tangency condition: **MRS = price ratio**! <br>

<br> Solving the system of equations given by the FOCs yields the optimal consumption bundle $(x^*, y^*)$ and the multiplier $\lambda^*$.
...
$$L = x^{\alpha}y^{\beta} + \lambda(I - p_x x - p_y y)$$

**Step 2**: Take FOCs
$$\frac{\partial L}{\partial x} = \alpha x^{\alpha-1}y^{\beta} - \lambda p_x = 0$$
$$\frac{\partial L}{\partial y} = \beta x^{\alpha}y^{\beta-1} - \lambda p_y = 0$$
...
**Step 3**: Solve the system of equations to find $x^*$, $y^*$, and $\lambda^*$.
...
# Application: Cash vs In-Kind Transfers
...
## Cash vs Food Stamps on a Graph
```{python}
#| echo: false
import numpy as np
import matplotlib.pyplot as plt
...
colors = ['#1e5288', '#16a085', '#f39c12', '#e74c3c', '#8e44ad', '#2c3e50']

# Original budget
x_orig = np.linspace(0, I/px, 100)
y_orig = (I - px*x_orig) / py
ax.plot(x_orig, y_orig, colors[0], linewidth=2.5, linestyle='--', label=f'Original Budget: {px}x + {py}y = {I}', alpha=0.7)

# Cash transfer budget
x_cash = np.linspace(0, (I+transfer)/px, 100)
y_cash = (I + transfer - px*x_cash) / py
ax.plot(x_cash, y_cash, colors[1], linewidth=3, label=f'Cash Transfer (${transfer})')
...
ax.plot(x_inkind, y_inkind, colors[2], linewidth=10, label=f'In-Kind Transfer (${transfer} vouchers for X)', alpha=0.35)

# Optimal with cash (assume Cobb-Douglas α=0.5)
alpha = 0.3
x_cash_opt = alpha * (I + transfer) / px
y_cash_opt = (1-alpha) * (I + transfer) / py
ax.plot(x_cash_opt, y_cash_opt, colors[1], markersize=10, label='Optimal with Cash', zorder=5, marker='o')
...
x_inkind_opt = x_min
y_inkind_opt = I / py
ax.plot(x_inkind_opt, y_inkind_opt, colors[2], markersize=10, label='Optimal with In-Kind', zorder=5, marker='o')

# Indifference curves
x_ic = np.linspace(0.5, 30, 300)

# IC through cash optimum
U_cash = x_cash_opt**alpha* y_cash_opt**(1-alpha)
y_ic_cash = (U_cash / x_ic**alpha)**(1/(1-alpha))
ax.plot(x_ic, y_ic_cash, colors[1], linewidth=2.5, linestyle='-')

# IC through in-kind optimum
U_inkind = x_inkind_opt**alpha* y_inkind_opt**(1-alpha)
y_ic_inkind = (U_inkind / x_ic**alpha) **(1/(1-alpha))
ax.plot(x_ic, y_ic_inkind, colors[2], linewidth=2.5)

# Mark the kink
# ax.plot(x_min, I/py, 'rs', markersize=10, zorder=5)
ax.axvline(x=x_min, color=colors[2], linestyle=':', alpha=0.5, linewidth=2)
ax.text(x_min-0.3, 12, f'Must buy\n≥{x_min:.0f} units', fontsize=10, color=colors[2], ha='right', fontweight='bold')
...
# add title to plot
ax.set_title('Cash vs In-Kind Transfers', fontsize=16)
...
3. **Common utility functions**: Perfect substitutes, perfect complements, Cobb-Douglas, CES
4. **Utility maximization**: Tangency condition (MRS = $p_x/p_y$), Lagrange method, Indirect utility
