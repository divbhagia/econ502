...
$$EU_B = 2 \times \frac{1}{2} + 3 \times \frac{1}{2} = 2.5 \quad \text{(using A's mix and either of B's strategies)}$$
...
def draw_line(ax, p1, p2):
    ax.plot([p1[0], p2[0]], [p1[1], p2[1]], 'k-', lw=1.5, zorder=1)

def label_edge(ax, p1, p2, label, left=True):
    mx, my = (p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2
    offset = -0.25 if left else 0.25
    ha = 'right' if left else 'left'
    ax.text(mx + offset, my, label, ha=ha, va='center', fontsize=10, zorder=4)

def draw_node(ax, xy, label):
    c = plt.Circle(xy, 0.3, color='white', ec='black', lw=1.8, zorder=3)
    ax.add_patch(c)
    ax.text(*xy, label, ha='center', va='center', fontsize=11, fontweight='bold', zorder=4)

def draw_leaf(ax, xy, text):
    ax.text(*xy, text, ha='center', va='center', fontsize=10, zorder=4,
            bbox=dict(boxstyle='round,pad=0.25', fc='#f8f8f8', ec='#888888', lw=0.8))

draw_line(ax, A, BL);  label_edge(ax, A, BL, 'U', left=True)
draw_line(ax, A, BR);  label_edge(ax, A, BR, 'D', left=False)
draw_line(ax, BL, UL); label_edge(ax, BL, UL, 'L', left=True)
draw_line(ax, BL, UR); label_edge(ax, BL, UR, 'R', left=False)
draw_line(ax, BR, DL); label_edge(ax, BR, DL, 'L', left=True)
draw_line(ax, BR, DR); label_edge(ax, BR, DR, 'R', left=False)
